...
export class AuthorizationService {
   ...
   // url=env.API_URL;
    ...
    constructor(
        ...
        @Inject('AuthorizationConfig') private environment: AuthorizationConfig,private http:HttpClient) {
        ...
        this._tokenResponses.subscribe((token: TokenResponse | null) => {
            if (token) {
                window.localStorage.setItem(LS_TOKEN_RESPONSE, JSON.stringify(token?token.toJson():''));
            } else {
                window.localStorage.removeItem(LS_TOKEN_RESPONSE);
            }
        });
                ...
                ([configuration, token]: [AuthorizationServiceConfiguration | null, TokenResponse | null]) => {
                    ...
                    if (this._userInfos.value == null) {
                        ...
                        this.requestor.xhr<UserInfo>({
                            ...
                            //this.nuId=window.localStorage.setItem('LS_USER_INFO',JSON.stringify(userinfo.sub));
                          //  window.localStorage.setItem('LS_USER_INFO',JSON.stringify(userinfo.sub));
                        });
                    }
                });
    ...
    //     const Url: string = `${this.url+env.applist}?${this.nuidId}${this.nuId}`;

    //     return this.http.get(Url);
    //    // console.log(this.http.get(this.url+'/applist'));
    //   }
    ...
    signOut(): void {
        ...
        localStorage.clear();
        window.location.href = this.environment.logoutUrl;
                    ...
                    this.notifier.setAuthorizationListener((request, response, error) => {
                        ...
                        if (response && response.code) {
                            ...
                            tokenHandler.performTokenRequest(configuration, tokenRequest)
                                ...
                                // if (this._userInfos.value == null) {
                                //     const accessToken = this._tokenResponses;
                                //     this.requestor.xhr<UserInfo>({
                                //         url: configuration.userInfoEndpoint,
                                //         method: 'GET',
                                //         dataType: 'json',
